...
class TestEnumChoiceMemberInitialization:
    ...
    def test_enum_member_choice_text_is_set_to_second_args_element_if_its_passed(
            self,
    ):
        choice_texts = ['First', '1', '7.4']
        ...
        for i, member in enumerate(EnumWithChoiceTexts):
            assert member.choice_text == choice_texts[i]

    def test_enum_member_choice_text_has_default_value_when_second_args_element_missing(
            self,
    ):
        choice_texts = ['Red', 'Dark Red', 'Super Dark Red']
